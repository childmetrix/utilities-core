...
create_rstudio_project <- function(base_path,
  ...
  if (file.exists(claude_template_path)) {
    ...
    # Patch the CLAUDE.md template with project name
    claude_lines <- readLines(claude_file, warn = FALSE)
    claude_lines <- gsub("\\[PROJECT_NAME\\]", project_name, claude_lines)
  ...
  # 7) Create or skip GitHub repo (using childmetrix organization)
  gh_owner <- "childmetrix"  # Use childmetrix organization
  gh_view_status <- suppressWarnings(
    run_in_project(sprintf("gh repo view %s/%s", gh_owner, project_name))
  )
  if (gh_view_status != 0) {
    gh_create_status <- run_in_project(
      sprintf('gh repo create %s/%s --private --confirm', gh_owner, project_name)
    )
